...
export async function registerRoutes(
  ...
  // اختبار مفتاح MIMO
  app.get("/api/mimo-test", (_req, res) => {
    res.json({
      hasKey: !!process.env.MIMO_API_KEY,
      keyPrefix: process.env.MIMO_API_KEY?.slice(0, 5) || null,
    });
  });
  ...
  app.post("/api/users", requireOwnerOrAdmin, async (req, res) => {
    ...
    if (pin) {
      ...
      if (pinUser)
        return res
          .status(409)
          .json({ message: "رقم PIN مستخدم بالفعل من موظف آخر" });
    }
  ...
  app.patch("/api/users/:id", requireOwnerOrAdmin, async (req, res) => {
    ...
    if (pin !== undefined) {
      if (pin) {
        const pinUser = await storage.getUserByPin(pin);
        if (pinUser && pinUser.id !== id)
          return res.status(409).json({ message: "رقم PIN مستخدم بالفعل" });
      }
    ...
    if (commissionRate !== undefined)
      updateData.commissionRate = String(commissionRate);
    if (employmentStatus !== undefined)
      updateData.employmentStatus = employmentStatus;
    if (openingAdvanceBalance !== undefined)
      updateData.openingAdvanceBalance = String(openingAdvanceBalance);
    if (openingPayableBalance !== undefined)
      updateData.openingPayableBalance = String(openingPayableBalance);
  ...
  app.get("/api/dashboard/executive", requireOwnerOrAdmin, async (req, res) => {
    try {
      ...
      const from =
        (req.query.from as string) || new Date().toISOString().slice(0, 10);
      const to =
        (req.query.to as string) || new Date().toISOString().slice(0, 10);
      ...
      const periodDays =
        Math.round(
          (new Date(to).getTime() - new Date(from + "T00:00:00").getTime()) /
            86400000,
        ) + 1;
      const prevToDate = new Date(from + "T00:00:00");
      ...
      branchExpQ.rows.forEach((r: any) => {
        branchExpMap[r.branch_id] = parseFloat(r.expenses);
      });
      ...
      const prevNet =
        parseFloat(prevSalesQ.rows[0].gross_profit) - prevExpenses;
      ...
      res.json({
        ...
        paymentSplit: paymentQ.rows.map((r) => ({
        ...
        branchPerformance: branchPerfQ.rows.map((r) => {
          ...
          return {
            ...
            avgInvoice: parseFloat(r.avg_invoice),
          };
        }),
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      `);
        ...
        const missingCogsPct =
          parseInt(mc.total_count) === 0
            ? 0
            : Math.round(
                (parseInt(mc.missing_count) / parseInt(mc.total_count)) * 10000,
              ) / 100;
        const missingCostPct =
          parseInt(mp.total_count) === 0
            ? 0
            : Math.round(
                (parseInt(mp.missing_count) / parseInt(mp.total_count)) * 10000,
              ) / 100;
        ...
        res.json({
          ...
          paymentSplit: paymentRes.rows.map((r) => ({
  ...
  app.patch(
    ...
    async (req, res) => {
      ...
      const row = await storage.updateCategory(
    ...
    async (req, res) => {
      try {
        ...
        if (!parsed.success)
          return res
            .status(400)
            .json({ message: formatZodError(parsed.error) });
        const { variants, ...productData } = parsed.data;
        const result = await storage.createProductWithVariants(
          productData as any,
          variants ?? [],
        );
        ...
        res
          .status(500)
          .json({ message: err?.message ?? "خطأ في إنشاء المنتج" });
      }
    },
  );
  app.patch(
    ...
    async (req, res) => {
      ...
      if (
        ...
        (oldProduct.price as any) !== parsed.data.price
      ) {
        storage
          .addAuditLog({
    ...
    async (req, res) => {
      try {
        ...
        if (parsed.data.barcode) {
          const existing = await storage.getVariantByBarcode(
            parsed.data.barcode,
          );
          if (existing)
  ...
  app.get("/api/variants/barcode/:barcode", requireAuth, async (req, res) => {
    const variant = await storage.getVariantByBarcode(
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        if (!variant)
          return res.status(404).json({ message: "المتغير غير موجود" });
        res.json(variant);
      } catch (err: any) {
        res.status(500).json({ message: err?.message ?? "خطأ" });
      }
  ...
  app.get(
    "/api/transfer-source-stock/:locationId",
    requireAuth,
    async (req, res) => {
      const locationId = Number(req.params.locationId);
      const locRes = await pool.query(
        `SELECT id, is_central, branch_id FROM locations WHERE id = $1`,
        [locationId],
      );
      ...
      if (loc.is_central) {
        ...
        const branchLocs = await pool.query(
          `SELECT id FROM locations WHERE branch_id = $1 AND active = true`,
          [loc.branch_id],
        );
  ...
  app.get("/api/stock-transfers/:id", requireAuth, async (req, res) => {
    ...
    const detFromLocRes = await pool.query(
      `SELECT id, is_central, branch_id FROM locations WHERE id = $1`,
      [transfer.fromLocationId],
    );
    ...
    async (req, res) => {
      try {
        ...
        if (!transfer || transfer.status !== "draft")
          return res.status(400).json({ message: "لا يمكن التعديل" });
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const result = await storage.approveStockTransfer(
          transfer.id,
          req.jwtUser!.userId,
        );
        if (!result) {
      ...
      `,
        ...
        res
          .status(400)
          .json({ message: err?.message ?? "خطأ في تنفيذ التحويل" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!result)
          return res.status(400).json({ message: "لا يمكن اعتماد التحويل" });
        res.json(result);
      } catch (err: any) {
        res
          .status(400)
          .json({ message: err?.message ?? "خطأ في اعتماد التحويل" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      if (req.query.locationId)
        filters.locationId = Number(req.query.locationId);
    ...
    async (req, res) => {
      ...
      const branchId = req.query.branchId
        ...
        : user.role === "owner" || user.role === "admin"
          ...
          : (user.branchId ?? undefined);
      res.json(
        await storage.getInventoryTransactions({
          branchId,
  ...
  app.get(
    "/api/location-inventory/transactions",
    requireAuth,
    async (req, res) => {
      const user = await storage.getUser(req.jwtUser!.userId);
      if (!user) return res.status(401).json({ message: "غير مصرح" });
  ...
  app.get(
    "/api/location-inventory/low-stock",
    requireAuth,
    async (req, res) => {
      const user = await storage.getUser(req.jwtUser!.userId);
      if (!user) return res.status(401).json({ message: "غير مصرح" });
  ...
  app.post(
    "/api/location-inventory/transfer",
    requireAuth,
    requireManager,
    async (req, res) => {
      try {
        const { fromLocationId, toLocationId, productId, quantity, note } =
          req.body;
        if (
          !fromLocationId ||
          !toLocationId ||
          ...
          return res
            .status(400)
            .json({ message: "البيانات ناقصة أو غير صحيحة" });
        }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const result = await storage.createLocationTransfer(
          branchId,
          items,
          req.jwtUser!.userId,
        );
  ...
  app.post(
    "/api/location-inventory/add-stock",
    requireAuth,
    requireManager,
    async (req, res) => {
      try {
  ...
  app.get("/api/customers/kpis", requireAuth, async (_req, res) => {
    try {
      res.json(await storage.getCustomerKpis());
    } catch (err: any) {
  ...
  app.post("/api/customers", requireAuth, async (req, res) => {
    try {
      ...
      if ((parsed.data as any).phone) {
        const existing = await storage.getCustomerByPhone(
          (parsed.data as any).phone,
        );
  ...
  app.post("/api/suppliers", requireAuth, requireManager, async (req, res) => {
    ...
    const existing = await storage.getSupplierByName(
      (parsed.data as any).name.trim(),
    );
    if (existing) {
    ...
    async (req, res) => {
      try {
        ...
        journalForSupplierPayment({
          ...
          console.error("[AutoJournal] Supplier payment error:", err.message),
        );
      ...
      if ((parsed.data as any).customerId) {
        storage
          .updateCustomerAfterSale(
            (parsed.data as any).customerId,
            String((parsed.data as any).total || "0"),
          )
          .catch(() => {});
      }
      journalForSale({
        ...
        console.error("[AutoJournal] Sale error:", err.message),
      );
  ...
  app.post("/api/orders", requireAuth, async (req, res) => {
    try {
      ...
      const order = await storage.createOrder(
        { ...parsed.data, shiftId, employeeId: req.jwtUser!.userId },
        items,
      );
      if ((parsed.data as any).customerPhone) {
        storage
          .findOrCreateCustomerByPhone(
            (parsed.data as any).customerPhone,
            (parsed.data as any).customerName || undefined,
          )
          .catch(() => {});
      }
    ...
    async (req, res) => {
      try {
        ...
        if (!statusParsed.success)
          return res
            .status(400)
            .json({ message: formatZodError(statusParsed.error) });
        const { status } = statusParsed.data;
        const existing = await storage.getOrder(Number(req.params.id));
        if (!existing)
          return res.status(404).json({ message: "الطلب غير موجود" });
        const oldStatus = existing.status;
        const row = await storage.updateOrderStatus(
          Number(req.params.id),
          status,
        );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const result = await storage.cancelOrderFull(
          orderId,
          jwtU.userId,
          jwtU.userName,
          reason,
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const retNumRes = await pool.query(
          `SELECT coalesce(max(id), 0) + 1 as next FROM sale_returns`,
        );
        ...
        journalForSaleReturn({
          ...
          console.error("[AutoJournal] Return error:", err.message),
        );
        ...
        res
          .status(500)
          .json({ message: err?.message ?? "خطأ في إنشاء المرتجع" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const rows = await storage.getExpensesEnriched(
          branchId,
          dateStr,
          fromDate,
          toDate,
    ...
    async (req, res) => {
      try {
        ...
        if (!user || !user.branchId) {
          return res
            .status(400)
            .json({ message: "بيانات المستخدم ناقصة (الفرع)" });
        }
        ...
        journalForExpense({
          ...
          console.error("[AutoJournal] Expense error:", err.message),
        );
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      if (!user || !user.branchId || !user.terminalName) {
        return res
          .status(400)
          .json({ message: "بيانات المستخدم ناقصة (الفرع أو الجهاز)" });
      }
      const shift = await storage.getCurrentShift(
        user.branchId,
        user.terminalName,
  ...
  app.post("/api/shifts", requireAuth, async (req, res) => {
    ...
    const existing = await storage.getCurrentShift(
      user.branchId,
      user.terminalName,
    );
    if (existing) {
  ...
  app.patch(
    ...
    async (req, res) => {
      ...
      if (actualCash === undefined || actualCash === null) {
        return res
          .status(400)
          .json({ message: "يجب إدخال المبلغ النقدي الفعلي (actualCash)" });
      }
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      if (!dateStr || !/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
        return res
          .status(400)
          .json({ message: "التاريخ مطلوب بصيغة YYYY-MM-DD" });
      }
      const scope = req.branchScope!;
      const branchId = scope.mode === "branch" ? scope.branchId! : undefined;
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "from & to required (YYYY-MM-DD)" });
      }
      const scope = req.branchScope!;
      const branchId = req.query.branchId
        ? Number(req.query.branchId)
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      res.json(
        await storage.getSalesListReport(from, to, branchId, paymentMethod),
      );
    },
  );

  app.get(
    "/api/reports/categories-report",
    requireAuth,
    enforceBranchScope,
    async (req, res) => {
      const from = req.query.from as string;
  ...
  app.get(
    "/api/reports/shift-details/:id",
    requireAuth,
    enforceBranchScope,
    async (req, res) => {
      const shiftId = Number(req.params.id);
      if (!shiftId)
        return res.status(400).json({ message: "shiftId required" });
  ...
  app.get(
    "/api/reports/branch-comparison-range",
    requireOwnerOrAdmin,
    async (req, res) => {
      const from = req.query.from as string;
      const to = req.query.to as string;
  ...
  app.get(
    "/api/reports/employee-performance/:id",
    requireOwnerOrAdmin,
    async (req, res) => {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "التاريخ مطلوب بصيغة YYYY-MM-DD (from & to)" });
      }
  ...
  app.patch(
    ...
    async (req, res) => {
      ...
      if (invoice.status !== "pending") {
        return res
          .status(400)
          .json({ message: "لا يمكن تعديل فاتورة معتمدة أو ملغاة" });
      }
      const updateData: any = {};
      const {
        shippingCost,
      ...
      if (shippingCost !== undefined)
        updateData.shippingCost = String(shippingCost);
      if (customsCost !== undefined)
        updateData.customsCost = String(customsCost);
      if (clearanceCost !== undefined)
        updateData.clearanceCost = String(clearanceCost);
    ...
    async (req, res) => {
      try {
        ...
        if (!invoice)
          return res
            .status(404)
            .json({ message: "فاتورة المشتريات غير موجودة" });
        if (invoice.status !== "pending")
          return res
            .status(400)
            .json({ message: "لا يمكن إضافة أصناف لفاتورة معتمدة أو مستلمة" });
  ...
  app.patch(
    ...
    async (req, res) => {
      ...
      if (invoice.status !== "pending") {
        return res
          .status(400)
          .json({ message: "لا يمكن تعديل أصناف فاتورة معتمدة أو ملغاة" });
      }
      ...
      const updated = await storage.updatePurchaseItem(itemId, {
        ...
        unitCostBase:
          unitCostBase !== undefined ? Number(unitCostBase) : undefined,
      });
      if (!updated) return res.status(404).json({ message: "الصنف غير موجود" });

      if (
        updated.variantId &&
        (barcode !== undefined || color !== undefined || size !== undefined)
      ) {
  ...
  app.delete(
    ...
    async (req, res) => {
      ...
      if (invoice.status !== "pending") {
        return res
          .status(400)
          .json({ message: "لا يمكن حذف أصناف من فاتورة معتمدة أو ملغاة" });
      }
      ...
      try {
        ...
        if (parsed.data.status === "approved") {
          if (invoice.status !== "pending")
            return res
              .status(400)
              .json({ message: "يمكن اعتماد الفواتير بحالة (pending) فقط" });
          ...
          journalForPurchase({
            ...
            console.error("[AutoJournal] Purchase error:", err.message),
          );
        ...
        if (parsed.data.status === "received") {
          if (invoice.status !== "approved")
            return res
              .status(400)
              .json({ message: "يجب اعتماد الفاتورة أولاً قبل الاستلام" });
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const result = await storage.approvePurchaseInvoice(
          Number(req.params.id),
        );
  ...
  app.post(
    "/api/purchases/:purchaseId/invoice-image",
    requireAuth,
    uploadLimiter,
    upload.single("file"),
    async (req: any, res) => {
  ...
  app.post(
    "/api/purchases/:purchaseId/parse-invoice",
    requireAuth,
    requireManager,
    async (req, res) => {
      try {
        ...
        const invoice = await storage.getPurchaseInvoice(purchaseId);
        let template:
          | { tableStartKeyword?: string | null; columnOrder?: string | null }
          | undefined;

        if (invoice?.supplierId) {
          const saved = await storage.getSupplierOcrTemplate(
            invoice.supplierId,
          );
          if (saved) {
            template = {
        ...
        if (
          result.ok &&
          result.parsed.detectedTemplate &&
          invoice?.supplierId
        ) {
        ...
        res.json({
          ok: false,
          stage: "parse",
          error: err?.message ?? "فشل قراءة الفاتورة",
        });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const result = await storage.receivePurchaseInvoice(
          Number(req.params.id),
        );
        res.json(result);
      } catch (err: any) {
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      const branchId =
        req.branchScope!.mode === "branch"
          ? (req.branchScope!.branchId ?? undefined)
          : req.query.branchId
            ? Number(req.query.branchId)
            : undefined;
    ...
    async (req, res) => {
      try {
        ...
        if (!amount || parseFloat(amount) <= 0)
          return res
            .status(400)
            .json({ message: "المبلغ مطلوب ويجب أن يكون أكبر من صفر" });
        const user = await storage.getUser(req.jwtUser!.userId);
        if (!user || !user.branchId)
          return res.status(400).json({ message: "بيانات المستخدم ناقصة" });
        let shiftId: number | null = null;
        if (user.terminalName) {
          const shift = await storage.getCurrentShift(
            user.branchId,
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      const filterDate = date
        ? String(date)
        : new Date().toISOString().slice(0, 10);
      const closedShifts = await storage.getClosedShiftsByDate(
        branchId,
        filterDate,
      );
  ...
  app.post("/api/stocktakes", requireAuth, requireManager, async (req, res) => {
    try {
      ...
      if (!branchId || !locationId)
        return res.status(400).json({ message: "الفرع والموقع مطلوبان" });
    ...
    async (req, res) => {
      try {
        ...
        if (countedQty === undefined || countedQty === null)
          return res.status(400).json({ message: "الكمية المعدودة مطلوبة" });
        const updated = await storage.updateStocktakeItem(
          Number(req.params.id),
          Number(countedQty),
          note,
        );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const updated = await storage.approveStocktake(
          Number(req.params.id),
          req.jwtUser!.userId,
        );
        if (!updated)
          return res.status(400).json({ message: "لا يمكن اعتماد هذا الجرد" });
        res.json(updated);
      } catch (err: any) {
        res.status(500).json({ message: err?.message ?? "خطأ في الخادم" });
      }
    ...
    async (req, res) => {
      try {
        ...
        const locationId = req.query.locationId
          ? Number(req.query.locationId)
          : undefined;
        const list = await storage.getInventoryAdjustments(
          branchId,
          locationId,
        );
        res.json(list);
    ...
    async (req, res) => {
      try {
        ...
        if (qtyAfter < 0)
          return res
            .status(400)
            .json({ message: "الكمية الناتجة لا يمكن أن تكون سالبة" });
  ...
  app.get(
    "/api/payroll/remaining-by-employee",
    requireAuth,
    requireManager,
    async (_req, res) => {
      try {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res.json(
          await storage.getEmployeeCommissions(
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const { employeeId, month, year } = req.query;
        res.json(
          await storage.getEmployeeEntitlements(
            employeeId ? Number(employeeId) : undefined,
            month ? String(month) : undefined,
            year ? Number(year) : undefined,
          ),
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!month || !year)
          return res.status(400).json({ message: "الشهر والسنة مطلوبة" });
  ...
  app.post(
    "/api/payroll-runs/:id/regenerate",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
        ...
        if (run.status !== "draft")
          return res
            .status(400)
            .json({ message: "لا يمكن إعادة احتساب كشف معتمد" });
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const updated = await storage.approvePayrollRun(
          Number(req.params.id),
          req.jwtUser!.userId,
        );
        if (!updated)
          return res.status(400).json({ message: "لا يمكن اعتماد هذا الكشف" });
        await storage.addAuditLog({
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const updated = await storage.reviewPayrollRun(
          Number(req.params.id),
          req.jwtUser!.userId,
        );
        if (!updated)
          return res.status(400).json({ message: "لا يمكن مراجعة هذا الكشف" });
        await storage.addAuditLog({
    ...
    async (req, res) => {
      try {
        ...
        if (!user || user.role !== "owner")
          return res
            .status(403)
            .json({ message: "فقط المالك يمكنه إعادة فتح الكشف" });
        const updated = await storage.reopenPayrollRun(
          Number(req.params.id),
          req.jwtUser!.userId,
        );
        if (!updated)
          return res
            .status(400)
            .json({ message: "لا يمكن إعادة فتح هذا الكشف" });
        await storage.addAuditLog({
    ...
    async (req, res) => {
      try {
        ...
        if (!user || user.role !== "owner")
          return res
            .status(403)
            .json({ message: "فقط المالك يمكنه إلغاء الكشف" });
        const updated = await storage.cancelPayrollRun(
          Number(req.params.id),
          req.jwtUser!.userId,
        );
        if (!updated)
          return res.status(400).json({ message: "لا يمكن إلغاء هذا الكشف" });
        await storage.addAuditLog({
    ...
    async (req, res) => {
      try {
        ...
        await storage.createEmployeeLedgerEntry({
          employeeId: Number(employeeId),
          movementType: "advance_given",
          referenceType: "employee_advance",
          referenceId: advance.id,
          amount: String(amount),
          date,
          note: note || "سلفة جديدة",
          createdBy: req.jwtUser!.userId,
        });
        ...
        await storage.addAuditLog({
          ...
          userId: req.jwtUser!.userId,
          userName: req.jwtUser?.userName || "",
          details: `إنشاء سلفة بمبلغ ${amount}`,
      ...
      try {
        ...
        if (deductionType === "recurring" && monthReference) {
          ...
          if (existingDup.rows.length > 0) {
            return res
              .status(400)
              .json({ message: "يوجد خصم متكرر بنفس السبب لنفس الفترة" });
          }
        }
  ...
  app.get(
    "/api/payroll-runs/:id/details-with-payments",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
        const details = await storage.getPayrollDetailsWithPayments(
          Number(req.params.id),
        );
        res.json(details);
      } catch (err: any) {
  ...
  app.get(
    "/api/employees/:id/financial-profile",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
        const profile = await storage.getEmployeeFinancialProfile(
          Number(req.params.id),
        );
        if (!profile)
  ...
  app.get(
    "/api/payroll/advances-outstanding",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
    ...
    async (req, res) => {
      try {
        ...
        if (run && !["approved", "partial", "reviewed"].includes(run.status)) {
          return res
            .status(400)
            .json({ message: "لا يمكن الدفع إلا لكشف معتمد أو تمت مراجعته" });
        }
        ...
        if (!detail.rows[0])
          return res.status(404).json({ message: "تفاصيل الراتب غير موجودة" });

        const existingPayments = await pool.query(
          `SELECT COALESCE(SUM(amount::numeric), 0) as total FROM salary_payments WHERE payroll_detail_id = $1`,
          [payrollDetailId],
        );
  ...
  app.get(
    "/api/payroll-detail/:id/payments",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
        const payments = await storage.getPayrollDetailPayments(
          Number(req.params.id),
        );
        res.json(payments);
      } catch (err: any) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        if (!month || !year)
          return res.status(400).json({ message: "الشهر والسنة مطلوبة" });
        const preview = await storage.previewPayrollRun(
          String(month),
          Number(year),
        );
        res.json(preview);
    ...
    async (req, res) => {
      try {
        ...
        if (req.query.movementType)
          filters.movementType = String(req.query.movementType);
        const ledger = await storage.getEmployeeLedger(
          Number(req.params.id),
          filters,
        );
        res.json(ledger);
  ...
  app.get(
    "/api/reports/employee-statement/:id",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
        ...
        const statement = await storage.getEmployeeStatement(
          Number(req.params.id),
          from,
          to,
        );
  ...
  app.get(
    "/api/reports/recurring-deductions",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        if (!month || !year)
          return res.status(400).json({ message: "الشهر والسنة مطلوبة" });
        const report = await storage.getPayrollByBranch(
          String(month),
          Number(year),
        );
        res.json(report);
  ...
  app.get(
    "/api/reports/payroll-comparison",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
  ...
  app.post(
    "/api/employees/:id/opening-balances",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
        ...
        if (!user || user.role !== "owner")
          return res
            .status(403)
            .json({ message: "فقط المالك يمكنه تعديل الأرصدة الافتتاحية" });
        ...
        const updated = await storage.updateUser(Number(req.params.id), {
          openingAdvanceBalance:
            openingAdvanceBalance !== undefined
              ? String(openingAdvanceBalance)
              : undefined,
          openingPayableBalance:
            openingPayableBalance !== undefined
              ? String(openingPayableBalance)
              : undefined,
        });
        if (!updated)
          return res.status(404).json({ message: "الموظف غير موجود" });
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        const acc = await storage.updateAccount(
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const entry = await storage.getJournalEntry(
          parseInt(req.params.id as string),
        );
    ...
    async (req, res) => {
      try {
        ...
        if (Math.abs(totalDebit - totalCredit) > 0.01) {
          return res
            .status(400)
            .json({ message: "القيد غير متوازن - المدين لا يساوي الدائن" });
        }
  ...
  app.post(
    "/api/journal-entries/generate-retroactive",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
        ...
        for (const sale of salesRes.rows) {
          const existing = await pool.query(
            `SELECT id FROM journal_entries WHERE source_type = 'sale' AND source_id = $1`,
            [sale.id],
          );
          if (existing.rows.length === 0) {
        ...
        for (const exp of expensesRes.rows) {
          const existing = await pool.query(
            `SELECT id FROM journal_entries WHERE source_type = 'expense' AND source_id = $1`,
            [exp.id],
          );
          if (existing.rows.length === 0) {
        ...
        const purchasesRes = await pool.query(
          `SELECT * FROM purchase_invoices WHERE status IN ('approved', 'received') ORDER BY id`,
        );
        for (const pur of purchasesRes.rows) {
          const existing = await pool.query(
            `SELECT id FROM journal_entries WHERE source_type = 'purchase' AND source_id = $1`,
            [pur.id],
          );
          if (existing.rows.length === 0) {
            await journalForPurchase({
              ...
              invoiceDate:
                pur.invoice_date || new Date().toISOString().slice(0, 10),
            });
            generated++;
          }
        }
      ...
      `);
        for (const ret of returnsRes.rows) {
          const existing = await pool.query(
            `SELECT id FROM journal_entries WHERE source_type = 'return' AND source_id = $1`,
            [ret.id],
          );
          if (existing.rows.length === 0) {
        ...
        res.json({
          success: true,
          generated,
          message: `تم توليد ${generated} قيد محاسبي بأثر رجعي`,
        });
      } catch (err: any) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        if (!accountId)
          return res.status(400).json({ message: "يجب تحديد الحساب" });
        const entries = await storage.getGeneralLedger(
          accountId,
  ...
  app.patch(
    "/api/payroll/ui/movements/:table/:id/cancel",
    requireAuth,
    requireOwnerOrAdmin,
    async (req, res) => {
      try {
